...
import byow.Core.BSPTree;
import byow.Core.Engine;
import byow.Core.Tile;
...
public class TERenderer {
    ...
    public void renderFrame(TETile[][] world, Tile player, BSPTree tree) {
        if (tree.light_on) renderFrameHelper(world, player, true);
        else renderFrameHelper(world, player, false);
    }
...
//        StdDraw.show();
    ...
    public void renderFrameHelper(TETile[][] world, Tile player, boolean light) {
        if (!light) {
            int numXTiles = world.length;
            int numYTiles = world[0].length;
            StdDraw.clear(new Color(0, 0, 0));
            for (int x = 0; x < numXTiles; x += 1) {
                for (int y = 0; y < numYTiles; y += 1) {
                    ...
                    if (((x - player.x) * (x - player.x) + (y - player.y) * (y - player.y)) <= 20) {
                        world[x][y].draw(x + xOffset, y + yOffset);
                    }
                }
            }
